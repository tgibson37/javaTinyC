...
public class ST extends PT {
    private Expr ex = Expr.getInstance();
...
System.err.println("    ST~93, xreturn");
            ...
            if ( eos ) {
                ...
                ex.asgn();  /* specified return value */
            }
...
System.err.println("    ST~113, xbreak");
        ...
        else if( ex.asgn() ) {      /* if expression discard its value */
System.err.println("    ST~118, ex.asgn");
            stk.toptoi();
            lit(xsemi);
        }
        else {
System.err.println("    ST~123, eset");
    ...
    public void varAlloc(Stuff.Type type, Stuff vpassed) {
        ...
        int alen=1;
        if( !ex.symName() ) {             /*defines fname,lname. True is match.*/
            eset(SYMERR);
            return;
        }
        cursor=lname+1;
        if( lit("(") ) {
            ...
            int ln=lname;
            if( ex.asgn() ) alen=stk.toptoi()+1;  /* dimension */
