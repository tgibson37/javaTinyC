 ...
 */ 
	...
	void setArg( Stuff.Type type, int arg ) {
/*
...
*/
    ...
    public void enter(int where) {   // c code tc~307
        ...
        if ( haveArgs ) {
System.err.println("Expr~40, cursor-->"+pr.substring(cursor,cursor+9));
            do {
                if(error!=0)return;
                if( asgn()) ++nargs;
...
// */
		if(error==0)stmt.st();     //  <<-- execute fcn's body
		else {
//			whatHappened();
			System.exit(1);
		}
