...
//	if(quit())return;
	...
	if(decl()){
		...
		if( !Expr.asgn() )return;   /* error */
		lit(xrpar);
		boolean condition = Stack.toptoi()!=0;
		if( condition ) {
/* prepare for repeating/skipping while (stcurs) 
	or object */ 
		...
		if ( eos ) {
			Stack.pushzero(); /* default return value */
		}
		else {
			Expr.asgn();  /* specified return value */
		}
	...
	else if( Expr.asgn() ) {      /* if expression discard its value */
		Stack.toptoi();
        lit(xsemi);
	}
	else {
